...
private:
	...
	std::string     account;
        ...
        EOSLIB_SERIALIZE(migrate, (id)(target)(account))
 ...
 private:
    ...
    inline uint64_t getmigratecount(){
	return toInt(getsysparam(MIGRATE_COUNT));
    }
 ...
 public:
    ...
    // @abi action
    void transmk(const uint64_t id, const account_name newowner, const std::string memo);
    ...
    // @abi action
    void applymigrate(const uint64_t id, const std::string target, const std::string account);
	...
	setsysparam(MIGRATE_COUNT, "MIGRATE_COUNT", std::to_string(max(0, getmigratecount() - 1)));
